...
| 8081 | Search API (Java) — BM25 + kNN endpoint |
| 9200 | OpenSearch — document store |
| 8083 | Embedding service (sentence-transformers all-MiniLM-L6-v2) |
| 11434 | Ollama — LLM |
| 5432 | PostgreSQL — Search API metadata |
...
| File | Responsibility |
|---|---|
| `main.py` | FastAPI app, route declarations, request models |
| `agent.py` | RAG pipeline: embed → hybrid search → rerank → LLM generate |
| `chat_handler.py` | Session-aware conversation history management |
| `tools.py` | Live tools: `get_logs()`, `get_pod_status()`, `describe_product()` |
...
| `entity_extractor.py` | NLP entity extraction from queries (environment name, product, error codes) |
| `resolver.py` | Resolves aliases → canonical customer ID |
| `session.py` | In-memory conversation session store with TTL expiry |
...
Query
  ...
  │       └─ environment name? product? ticket ID? error code?
  │
  ├─ 2. Embedding
  │       └─ POST /embed → embedding-service → all-MiniLM-L6-v2
  │
  ├─ 3. Hybrid search (BM25 + kNN)
  │       └─ POST /api/v1/search → search-api → OpenSearch
  │           ├─ BM25 (keyword) + kNN (semantic, HNSW) with RRF fusion
  │           └─ Filtered by: tenant, product, customer tags (when specified)
  │
  ├─ 4. Live tool calls (if query requires real-time data)
  │       └─ tools.py → ES logs / k8s state / pod status
  │
  ├─ 5. Context assembly
  │       └─ top-K chunks + live tool output + conversation history
  │
  └─ 6. LLM generation
          └─ POST http://ollama:11434/api/generate (streamed)
```
...
| [0016](adr/0016-hybrid-bm25-knn-rag.md) | Search mode | BM25 + kNN hybrid (RRF) | Keyword search catches ticket IDs / error codes; semantic search catches intent |
...
1. Entity extraction:  environment=acme-staging, product=backend
2. Embedding:          embed("service X failing") → 384-dim vector
3. Hybrid search:      BM25("service X failing") + kNN(vector)
                       → top-8 chunks from Confluence/GitHub/Jira
4. Live tool:          get_logs(customer=acme-staging, query="error", tail=200)
                       → Mode A: SSH bastion → kubectl exec filebeat-pod → ES query
                       → recent log lines
5. Context:            chunks (§3) + log lines (§4) + conversation history
6. LLM:                generates answer with citations
7. Response:           streamed to browser
```
...
|---|---|---|
| LLM throughput | ~20-40s/query, 1 concurrent | Add GPU node; or add second Ollama instance + round-robin |
| Embedding | ~50ms/embed | Multi-worker FastAPI; larger model with GPU |
